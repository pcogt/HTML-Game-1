...
function component(width, height, color, x, y) {
    ...
    this.newPos = function() {
        ...
        if(this.x + this.speedX > 0 && this.x + this.speedX <= defaultCanvasWidth - 30) {
            this.x += this.speedX;
        }
        else {
            if(this.x + this.speedX > defaultCanvasWidth - 30) {
                this.x = defaultCanvasWidth - 30;
            }
...
var myGameArea = {
    ...
    start : function() {
        ...
        this.interval = setInterval(updateGameArea, 10); // updates the canvas every 10ms
...
function updateGameArea() {  // only allow the GamePiece to move left and right on the canvas
    myGameArea.clear();
